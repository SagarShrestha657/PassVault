...
export const sendVerificationEmail = async (recipientEmail, verificationCode) => {
  try {
    ...
    <a href="/emailverification" style="display: inline-block; margin-top: 20px; padding: 10px 20px; font-size: 16px; font-weight: bold; color: #2193b0; background: #ffffff; border-radius: 5px; text-decoration: none; box-shadow: 0 2px 5px rgba(0, 0, 0, 0.2);">
        Verify Now
    </a>
    
    <p style="margin-top: 20px; font-size: 14px;">Best regards,<br><strong>Login Manager</strong></p>
</div>

    `;

    const info = await transporter.sendMail({
      from: `"Login Manager" <${process.env.NODE_EMAIL}>`, // Sender email
      to: recipientEmail, // Dynamic recipient
      subject: "Your Verification Code for Login Manager", // Email subject
...
export const sendWelcomeEmail = async (recipientEmail) => {
    try {
      ...
      const emailHTML = `
        <div style="font-family: Arial, sans-serif; max-width: 500px; margin: auto; padding: 20px; background: linear-gradient(135deg, #6dd5ed, #2193b0); color: #ffffff; border-radius: 10px; text-align: center; box-shadow: 0 4px 10px rgba(0, 0, 0, 0.1);">
            <h1 style="margin-bottom: 10px;">🎉 Welcome to Login Manager! 🎉</h1>
            ...
            <p style="font-size: 14px; margin-top: 15px;">Best regards,<br><strong>Login Manager</strong></p>
        </div>
      `;
  
      const info = await transporter.sendMail({
        from: `"Lgin Manager" <${process.env.NODE_EMAIL}>`,
        to: recipientEmail,
        subject: "🎉 Welcome to Login Manager!",
        html: emailHTML,
      });
